				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	idac0 = data[7];
#else
	pr_debug(KERN_DEBUG "called %s data[6] =%d\n", __func__, data[6]);
	idac0 = data[6];
#endif
	return sprintf(buf, "%d\n", idac0);
}
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	idac1 = data[6];
#else
	pr_debug(KERN_DEBUG "called %s data[7] = %d\n", __func__, data[7]);
	idac1 = data[7];
#endif
	return sprintf(buf, "%d\n", idac1);
}
...
static int touchkey_firmware_update(struct touchkey_i2c *tkey_i2c)
{
	...
	if (data[0] == 0 && data[1] == 0) {
		pr_debug(KERN_ERR"[TouchKey] failed to update fw\n");
	...
	if ((tkey_i2c->firmware_ver < TK_FIRMWARE_VER) &&
		...
		pr_debug(KERN_DEBUG "[TouchKey] firmware auto update execute\n");
...
static irqreturn_t touchkey_interrupt(int irq, void *dev_id)
{
	...
	while (retry--) {
		...
		if (!ret) {
			...
			if (keycode_type <= 0
				...
				pr_debug(KERN_DEBUG "[Touchkey] keycode_type err"
					" ret:%d, retry: %d\n",
					keycode_type, retry);
				continue;
			}
	...
	if (keycode_type <= 0 || keycode_type >= touchkey_count) {
		int i;
		pr_debug(KERN_DEBUG "[Touchkey] keycode_type err %d."
			" Release all keys\n", keycode_type);
		/* release keys */
	...
	if (get_tsp_status() && pressed)
		pr_debug(KERN_DEBUG "[TouchKey] touch is pressed. Pass touchkey event\n");
	else {
		input_report_key(tkey_i2c->input_dev,
